...
We then run the model for different numbers of topic, and evaluate its accuracy (wich is measured by the model's perplexity).
We toggle this codeblock's execution off because it takes a long time to run, but the model is most accurate with 25 topics.
