...
IRAcast_train <-Tweets_Train %>%
  count(Tweet_n, word) %>%
  cast_dtm(Tweet_n, word, n)

IRAcast_test <- Tweets_Test %>%
  count(Tweet_n, word) %>%
  cast_dtm(Tweet_n, word, n)
...
IRA_trainLDA <- LDA(IRAcast_train, k = 4, method = "Gibbs") #"k = " for topics
trainLDA_tidy <- tidy(IRA_LDA, matrix = "beta")


IRAterms_train <- trainLDA_tidy %>%
  group_by(topic)%>%
  slice_max(beta, n = 10) %>%
  ungroup() %>%
  arrange(topic, beta)
...
perplexity(IRA_trainLDA, IRAcast_train, estimate_theta = F)
perplexity(IRA_trainLDA, IRAcast_test, estimate_theta = T )
```
