...
author: "Francesco Prem Solidoro[,](https://www.youtube.com/watch?v=dQw4w9WgXcQ&pp=ygUYbmV2ZXIgZ29ubmEgZ2l2ZSB5b3UgdXAg) Matteo Liguori, Michele Salvi"
format:
  typst:
    toc: true
...
-   Matteo Liguori / matteo.liguori2\@studio.unibo.it / 0001071940
...
-   is it possible to generalize and model an astroturfing from the words used?
    ...
    To understand this, we will be conducting a sentiment analysis over the datasets of IRA-affiliated tweets utilizing text mining practices such as word frequencies, tf-idf, sentiment analysis through the NRC @NRC, Bing @BING and AFINN @AFINN lexicons, and the aid of LDA, latent dirichlet allocation, to model the topics discussed in the tweets.
...
Before modelling with LDA, we try to gauge information with a simpler regression model.

The first regression models Followers given sentiment in words, account category and followers.

```{r}
# alternative way to create dummies for regression, but the lm() function already does
# Emotion <- factor(nrc_tweets$sentiment)
# emo_reg <- nrc_tweets %>%
#   mutate(dums = model.matrix(~Emotion))


#Modelling followers given other variables
  
lm1 <- lm(followers ~ sentiment + account_category + following , data = nrc_tweets)
lm1_summ <- summary(lm1)
mean(lm1_summ$residuals^2)
#MSE very large
lm1_summ
```

In this case we observe that the factor most responsible in predicting followers is accounts of the "NewsFeed" category, boasting an increase in followers, ceteris paribus, of 5.606 x10\^3. Still, this is associated with a MSE of \~43M.

Noting the high p-value of the anticipation sentiment and fearmonger category, we ran an additional regression removing those variables.

```{r}
nrc_adjust <-nrc_tweets  %>% 
  filter(!sentiment == "anticipation",
         !account_category == "Fearmonger")

# model removing low significance variables
lm1_adj <- lm(followers ~ sentiment + account_category + following , data = nrc_adjust)
lm1adj_summ <- summary(lm1_adj)
lm1adj_summ
mean(lm1adj_summ$residuals^2)

#So MSE increases by removing "anticipation" and "fearmonger"
```

With this change in the regressors we observe slightly different estimates for most of the regressors, (excluding following), the MSE rose from \~43M to \~45M, while the R\^2 stays essentially the same.

We then tried modelling an additional regression that predicts Interactions (updates) given followers, sentiments, account category and following.

```{r}
#Modelling interactions (updates) given other variables
lm2 <- lm(updates ~ followers + sentiment + account_category + following , data = nrc_tweets)

lm2_summ <- summary(lm2)
lm2_summ
mean(lm2_summ$residuals^2)
```

This regression has a very high variance (and low R\^2) and high MSE (\~384M), which tells us that this kind of regression dosen't posess a significant predictive power and explains only a small part of the variance.

Since most of the sentiments have a very low significance we tried to understand the differences by gradually excluding the sentiments with the lowest significance.

```{r}
#differences of including sentiments in interaction regression
lm5 <-  lm(updates ~ followers + sentiment + account_category + following, data = nrc_tweets)
lm5_summ <- summary(lm5)


nrc_nofear <- nrc_tweets %>%
  filter(!sentiment == "fear")

lm6 <-  lm(updates ~ followers + sentiment + account_category + following, data = nrc_nofear)
lm6_summ <- summary(lm6)


nrc_nofearsurp <- nrc_tweets %>%
  filter(!sentiment == "fear",
         !sentiment == "surprise")

lm7 <-  lm(updates ~ followers + sentiment + account_category + following, data = nrc_nofearsurp)
lm7_summ <- summary(lm7)


nrc_nofst <- nrc_tweets %>%
  filter(!sentiment == "fear",
         !sentiment == "surprise",
         !sentiment == "trust")

lm8 <-  lm(updates ~ followers + sentiment + account_category + following, data = nrc_nofst)
lm8_summ <- summary(lm8)

#so, taking into account MSE, it's  better to select a model that contains all variables + additional regressors

lm5_summ
mean(lm5_summ$residuals^2)

lm6_summ
mean(lm6_summ$residuals^2)

lm7_summ
mean(lm7_summ$residuals^2)

lm8_summ
mean(lm8_summ$residuals^2)
```

But, as it's clearly observed, as we removed sentiments with the lowest significance from the regression, the R\^2 lowered while the MSE rose, while the estimates were almost stable.

To obtain more results on the first regression we conducted variable selection via the stepwise backward selection algorithm.

```{r}
# making  categorical variable dummies

  tweets_dummies <- nrc_tweets %>%
    select(c(followers,following,sentiment,account_category)) %>%
    dummy_cols(select_columns = c("sentiment","account_category"), remove_selected_columns = T)

lm1_tot <- lm(followers ~ . , data = tweets_dummies)
lm1_tot_step <- lm1_tot %>%
  step()

lm1_tot_step_summ <- summary(lm1_tot_step)
lm1_tot_step_summ
mean(lm1_tot_step_summ$residuals^2)
```

The algorithm has set the best performing selection automatically, and we observed an identical R\^2 and a slightly higher MSE, but with different estimates that suggest even more the importance of being a "Newsfeed" account.

To further inspect the validity of the regression we run different diagnostic plots:

-   Residuals vs Fitted, checking if residuals are randomly scattered around 0
-   Normal Q-Q, checking if residuals follow a normal distribution
-   Scale - Location, checking if residual variance is constant across predicted values
-   Residuals vs Leverage, identifying points that have a disproportionate impact on the model
...
To attempt to account for the mistakes highlighted by the diagnostic plots we computed the regression again, but with logarithmic "followers" and "following". Since some accounts have 0 followers / following, a logarithmic transformation is possible only by excluding them.

```{r}

tweets_dummies_adj <- tweets_dummies %>%
  filter(!following == 0,
         !followers == 0)

lm1_tot_log <- lm(log(followers) ~ log(following) + . , data = tweets_dummies_adj)

lm1_tot_log_step <- lm1_tot_log %>%
  step()

lm1_log_summ <- summary(lm1_tot_log_step)
lm1_log_summ
mean(lm1_tot_step_summ$residuals^2)

```

By applying the logarithmic transformations the R\^2 improved considerably, despite the MSE staying the same, then we again ran the same diagnostic plots for this updated regression.
...
This improved regression has the best performing R\^2 (0.8), and unexpectedly identifies the "Commercial" account category as the most successful in follower count, as opposed to the "Newsfeed" category, that the last regressions indicated.

### LDA Modelling

We proceed to use LDA analysis @LDA to create a model that splits the tweets we found into topic. We take each tweet as a document and its text as a corpus. The tweets are then aggregated through LDA into topics, which returns us a model for how likely each word is to come out each topic.
...
It'd be interesting to observe the evolution of astroturf given the recent developments in generative technology. To do so, we'd need to access new, large amounts of data. This can't cheaply be done off of Twitter (now X), so we hope to do so through fediverse or bluseky (or AT-based) platforms.

With this new data, we could then run a similar kind of analysis to observe if the patterns we found in 2016 remain and fully develop a recognisation model. At the same time, we could better develop a model using the help of network analysis: should our hypothesis about polarization hold, increases in network polarization would be possible markers of astroturf campaigns, and therefore "hot spots" to analyse.
