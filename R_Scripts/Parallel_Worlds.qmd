...
mean_imp <- Test %>%
  select(c(author,updates)) %>% 
  group_by(author) %>%
  summarize(Mean_Impressions = round(mean(updates),digits = 3)) 
...
Lang_Plot <- Test %>% 
  ...
  slice(1:6)%>%
  ungroup() %>%
  ggplot(aes(reorder(language, count),log(count)))+
  geom_bar(stat = "identity",fill = "darkblue")+
  coord_flip()+
  theme_bw()+
  labs(x = "Language", y = "Tweets (Log)", title = "Most Significant Languages")
...
Region_Plot <-  Test %>% 
  ...
  slice(1:6)%>%
  ungroup() %>%
  ggplot(aes(reorder(region, count),log(count)))+
  geom_bar(stat = "identity", fill = "darkred")+
  coord_flip()+
  theme_bw()+
  labs(x = "Region", y = "Tweets (Log)", title = "Most Significant Regions")
